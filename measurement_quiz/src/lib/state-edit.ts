 ...
 * has settled.
 *
 * COPIED VERBATIM from quantum_sketch (src/state-edit.ts). Do not edit here —
 * fix it there and re-copy, or lift it into misty and delete both copies. It is
 * pure `(source, ...) -> Edit | null` with no DOM, which is why copying is
 * cheap; every function re-parses before returning, so an edit that would not
 * parse simply does not happen.
 */
