<script lang="ts">
  ...
  $effect(() => {
    ...
    geometry = (result.geometry as typeof geometry) ?? null;
    // The SVG is laid out in diagram units and scaled to fit its box, so a
    // caller placing an overlay needs the ratio, not the raw numbers.
    const svg = host.querySelector('svg');
    unit = svg && result.height ? svg.getBoundingClientRect().height / result.height : 1;
  });
  ...
  /**
   * Where a state sits after `layer` has acted, in pixels down this figure.
   *
   * Taken from misty's own circuit geometry rather than guessed, so an overlay
   * lands on the wire instead of near it. `-1` is the input row.
   */
  export function rowY(layer: number): number | null {
    if (!geometry) return null;
    if (layer < 0) return geometry.startY * unit;
    const row = geometry.layers[layer];
    if (!row) return geometry.endY * unit;
    return (row.y + row.h) * unit;
  }

  /** Horizontal centre of the wires, for centring an overlay. */
  export function centreX(): number | null {
    const svg = host?.querySelector('svg');
    if (!svg) return null;
